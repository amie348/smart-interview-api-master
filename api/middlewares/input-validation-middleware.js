...
// params, query) and returns a validation function which validates the
...
const validateInput = (inputSchema, schemaScope) => {
  ...
  // defining a method to validate path query of url for incoming request
  const requestPathQueryValidator = async (req, res, next) => {
  ...
  return schemaScope === BODY ? requestBodyValidator : schemaScope === PARAMS ? requestPathParamsValidator : schemaScope === QUERY ? requestPathQueryValidator : requestDummyValidator;
